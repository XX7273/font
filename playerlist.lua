...
-- Library init
    getgenv().library = {
        ...
        current_open; 
        current_input = nil;  -- 全局输入锁, 防止多指冲突
    }
...
-- Library functions 
    -- Misc functions
        ...
        function library:resizify(frame)
            ...
            -- 缩放时的黑底覆盖层 (参考 juju 库的 drag_frame)
            ...
            local overlay_inner = library:create("Frame", {
                Parent = overlay;
                Name = "\0";
                Size = dim2(1, -2, 1, -2);
            ...
            library:create("UICorner", {
                Parent = overlay_inner;
                CornerRadius = dim(0, 8);
            })

            local size_label = library:create("TextLabel", {
                Parent = overlay;
                Name = "\0";
                Size = dim2(1, 0, 1, 0);
                BackgroundTransparency = 1;
            ...
            local function show_overlay()
                ...
                overlay_inner.BackgroundTransparency = 1
                size_label.TextTransparency = 1
                library:tween(overlay, {BackgroundTransparency = 0}, Enum.EasingStyle.Quad, 0.1)
                library:tween(overlay_inner, {BackgroundTransparency = 0}, Enum.EasingStyle.Quad, 0.1)
                library:tween(size_label, {TextTransparency = 0.2}, Enum.EasingStyle.Quad, 0.1)
                -- 0.1s 后强制不透明, 防止缩放过程中内容透出闪烁
                task.delay(0.1, function()
                    if resizing then
                        overlay.BackgroundTransparency = 0
                        overlay_inner.BackgroundTransparency = 0
                    end
                end)
            end

            local function hide_overlay()
                library:tween(overlay, {BackgroundTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
                library:tween(overlay_inner, {BackgroundTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
                library:tween(size_label, {TextTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
            ...
            handle.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if library.current_input and library.current_input ~= handle then
                        return
                    end
                    library.current_input = handle
                    resizing = true
                    active_input = input
            ...
            library:connection(uis.InputEnded, function(input)
                ...
                or (active_input and active_input.UserInputType == Enum.UserInputType.MouseButton1
                    and input.UserInputType == Enum.UserInputType.MouseButton1) then
                    library.current_input = nil
                    resizing = false
                    active_input = nil
                    hide_overlay()
                end
            ...
            library:connection(uis.InputChanged, function(input)
                if not resizing then return end
                if library.current_input and library.current_input ~= handle then return end

                if active_input.UserInputType == Enum.UserInputType.MouseButton1 then
                    if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
                elseif active_input.UserInputType == Enum.UserInputType.Touch then
                    if input ~= active_input then return end
                else
                    return
                end
                ...
                size_label.Text = string.format("%d × %d", current_size.X.Offset, current_size.Y.Offset)
                -- 保持覆盖层完全不透明
                overlay.BackgroundTransparency = 0
                overlay_inner.BackgroundTransparency = 0
            end)
        ...
        function library:draggify(frame)
            ...
            local start 
            local active_input = nil  -- 记住具体哪个 InputObject

            frame.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    -- 全局输入锁: 如果已有其他输入活跃, 放弃
                    if library.current_input and library.current_input ~= frame then
                        return
                    end
                    library.current_input = frame  -- 占坑
                    dragging = true
                    active_input = input
                    start = input.Position
            ...
            -- 全局 InputEnded: 防止手指滑出 frame 时断触
            library:connection(uis.InputEnded, function(input)
                if not dragging then return end
                -- 精确匹配: 只在原始 InputObject 抬起时停止
                if input == active_input 
                or (active_input and active_input.UserInputType == Enum.UserInputType.MouseButton1 
                    and input.UserInputType == Enum.UserInputType.MouseButton1) then
                    library.current_input = nil
            ...
            library:connection(uis.InputChanged, function(input, game_event) 
                if not dragging then return end
                -- 全局输入锁检查
                if library.current_input and library.current_input ~= frame then return end

                if active_input.UserInputType == Enum.UserInputType.MouseButton1 then
                    if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
                elseif active_input.UserInputType == Enum.UserInputType.Touch then
                    -- Touch: 用 InputObject 引用精确匹配, 只跟踪第一根手指
                    if input ~= active_input then return end
                else
                    return
                end
    ...
    -- Library element functions
        function library:window(properties)
            local cfg = { 
                ...
                size = properties.size or properties.Size or dim2(0, 530, 0, 400);
                selected_tab;
                items = {};
            ...
            local items = cfg.items; do
                ...
                items[ "multi_holder" ] = library:create( "Frame" , {
                    ...
                    Name = "\0";
                    BackgroundTransparency = 1;
                    Position = dim2(0, 150, 0, 0);
                    BorderColor3 = rgb(0, 0, 0);
                    Size = dim2(1, -150, 0, 56);
                    BorderSizePixel = 0;
                    BackgroundColor3 = rgb(255, 255, 255)
                ...
                items[ "global_fade" ] = library:create( "Frame" , {
                    ...
                    Name = "\0";
                    BackgroundTransparency = 1;
                    Position = dim2(0, 150, 0, 56);
                    BorderColor3 = rgb(0, 0, 0);
                    Size = dim2(1, -150, 1, -81);
                    BorderSizePixel = 0;
                    BackgroundColor3 = rgb(14, 14, 16);
                    ZIndex = 2;
                });                
            ...
            function cfg.toggle_menu(bool) 
                ...
                -- library:tween(items[ "tab_holder" ], {Size = dim2(1, -150, 1, -81)}, Enum.EasingStyle.Quad, 0.4)
                -- cfg.tween = 
            ...
            function cfg.open_tab() 
                ...
                if selected_tab then 
                    if selected_tab[ 4 ] ~= items[ "tab_holder" ] then 
                        ...
                        library:tween(self.items[ "global_fade" ], {BackgroundTransparency = 1}, Enum.EasingStyle.Quad, 0.4)
                        selected_tab[ 4 ].Size = dim2(1, -170, 1, -75)
                    end
        ...
        function library:slider(options) 
            ...
            local active_input = nil
            local slider_frame = items["slider"]

            -- 拖拽时禁用页面 ScrollingFrame, 防止手指滑动时页面跟着滚
            local saved_scroll = {}
            local function disable_page_scroll()
                for _, desc in library["items"]:GetDescendants() do
                    if desc:IsA("ScrollingFrame") and desc.ScrollingEnabled then
                        saved_scroll[desc] = true
                        desc.ScrollingEnabled = false
                    end
                end
            end
            local function restore_page_scroll()
                for frame in saved_scroll do
                    frame.ScrollingEnabled = true
                end
                table.clear(saved_scroll)
            end

            slider_frame.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if library.current_input and library.current_input ~= slider_frame then
                        return
                    end
                    library.current_input = slider_frame
                    cfg.dragging = true
                    active_input = input
                    disable_page_scroll()
            ...
            library:connection(uis.InputChanged, function(input)
                ...
                if library.current_input and library.current_input ~= slider_frame then return end
        ...
        function library:dropdown(options) 
            ...
            function cfg.set_visible(bool)
                ...
                local dd_x = items[ "dropdown" ].AbsolutePosition.X
                local dd_y = items[ "dropdown" ].AbsolutePosition.Y + 80
                local dd_w = items[ "dropdown" ].AbsoluteSize.X
                local dd_h = a
                local vp_x = camera.ViewportSize.X
                local vp_y = camera.ViewportSize.Y
                -- clamp to viewport
                if dd_x + dd_w > vp_x then dd_x = vp_x - dd_w - 5 end
                if dd_x < 0 then dd_x = 5 end
                if dd_y + dd_h > vp_y then dd_y = items[ "dropdown" ].AbsolutePosition.Y - dd_h - 5 end
                if dd_y < 0 then dd_y = 5 end
                items[ "dropdown_holder" ].Position = dim2(0, dd_x, 0, dd_y)
        ...
        function library:colorpicker(options) 
            ...
            function cfg.set_visible(bool)
                ...
                local cp_x = items[ "colorpicker" ].AbsolutePosition.X
                local cp_y = items[ "colorpicker" ].AbsolutePosition.Y + items[ "colorpicker" ].AbsoluteSize.Y + 45
                local cp_w = 166
                local cp_h = 197
                local vp_x = camera.ViewportSize.X
                local vp_y = camera.ViewportSize.Y
                -- clamp to viewport
                if cp_x + cp_w > vp_x then cp_x = vp_x - cp_w - 5 end
                if cp_x < 0 then cp_x = 5 end
                if cp_y + cp_h > vp_y then cp_y = items[ "colorpicker" ].AbsolutePosition.Y - cp_h - 5 end
                if cp_y < 0 then cp_y = 5 end
                items[ "colorpicker_holder" ].Position = dim_offset(cp_x, cp_y)
            ...
            function cfg.update_color(input) 
                ...
                if input and input.UserInputType == Enum.UserInputType.Touch then
                    -- Touch 的 Position 是 Vector3, 需要转为 Vector2 才能和 AbsolutePosition 做减法
                    offset = vec2(input.Position.X, input.Position.Y)
                else
                    local m = uis:GetMouseLocation() 
                    offset = vec2(m.X, m.Y - gui_offset)
                end

                if current_drag == "sat" then
            ...
            -- 拖拽时禁用页面 ScrollingFrame, 防止手指碰到 section 时页面跟着滚
            local saved_scroll = {}
            local function disable_page_scroll()
                for _, desc in library["items"]:GetDescendants() do
                    if desc:IsA("ScrollingFrame") and desc.ScrollingEnabled then
            ...
            local function start_drag(drag_type, input)
                if library.current_input and library.current_input ~= cp_lock then
                    return
                end
                library.current_input = cp_lock
                current_drag = drag_type
                active_input = input
                disable_page_scroll()
            ...
            items[ "alpha_gradient" ].InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    start_drag("alpha", input)
                end
            end)

            items[ "hue_gradient" ].InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    start_drag("hue", input)
                end
            end)

            items[ "sat" ].InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    start_drag("sat", input)
                end
            end)

            library:connection(uis.InputChanged, function(input)
                if not current_drag then return end
                if library.current_input ~= cp_lock then return end
        ...
        function library:keybind(options) 
            ...
            function cfg.set(input)
                ...
                -- 更新 keybinds 列表
                if cfg.list_entry then
                    cfg.list_entry.update(cfg.active)
                end
            end
            ...
            function cfg.set_visible(bool)
                ...
                local kb_x = items[ "keybind_holder" ].AbsolutePosition.X
                local kb_y = items[ "keybind_holder" ].AbsolutePosition.Y + items[ "keybind_holder" ].AbsoluteSize.Y + 60
                local kb_w = items[ "keybind_holder" ].AbsoluteSize.X
                local kb_h = size
                local vp_x = camera.ViewportSize.X
                local vp_y = camera.ViewportSize.Y
                if kb_x + kb_w > vp_x then kb_x = vp_x - kb_w - 5 end
                if kb_x < 0 then kb_x = 5 end
                if kb_y + kb_h > vp_y then kb_y = items[ "keybind_holder" ].AbsolutePosition.Y - kb_h - 5 end
                if kb_y < 0 then kb_y = 5 end
                items[ "dropdown" ].Position = dim_offset(kb_x, kb_y)
            end
            ...
            items[ "keybind_holder" ].MouseButton1Down:Connect(function()
                press_start = tick()
                hold_timer = task.delay(0.5, function()
                    -- 长按 0.5s 打开模式选择 (替代右键)
                    cfg.open = not cfg.open 
                    cfg.set_visible(cfg.open)
                end)
            ...
            items[ "keybind_holder" ].MouseButton1Up:Connect(function()
                if hold_timer then 
                    task.cancel(hold_timer)
                    hold_timer = nil
                end
            end)
            ...
            -- 注册到 keybinds 列表
            cfg.list_entry = library:register_keybind(cfg.name or cfg.flag, cfg)
        ...
        function library:playerlist(properties)
            local cfg = {
                name = properties.name or properties.Name or "Playerlist";
                flag = properties.flag or library:next_flag();
                callback = properties.callback or function() end;
                list_height = properties.height or properties.Height or 160;
                items = {};
                players = {};
                current_player = nil;
            ...
            local status_colors = {
                ...
                Priority = rgb(235, 76, 48),
                Friendly = rgb(134, 235, 56),
            }

            local status_order = {"Neutral", "Priority", "Friendly"}

            -- Container (outline + inline, same as section but without title bar)
            items["outline"] = library:create("Frame", {
                Parent = self.items["column"];
                Name = "\0";
                Size = dim2(1, 0, 0, 0);
            ...
            -- Content holder (auto-stacks children vertically)
            ...
            -- Info panel (selected player details)
            items["info_panel"] = library:create("Frame", {
                Parent = items["content"];
                Name = "\0";
                Size = dim2(1, 0, 0, 56);
                BackgroundColor3 = rgb(33, 33, 35);
            ...
            items["userid"] = library:create("TextLabel", {
                FontFace = fonts.small;
                TextColor3 = rgb(130, 130, 130);
                BorderColor3 = rgb(0, 0, 0);
                Text = "None";
            ...
            items["account_age"] = library:create("TextLabel", {
                FontFace = fonts.small;
                TextColor3 = rgb(130, 130, 130);
                BorderColor3 = rgb(0, 0, 0);
                Text = "None";
            ...
            -- Status selector
            items["status_holder"] = library:create("Frame", {
                Parent = items["content"];
                Name = "\0";
                Size = dim2(1, 0, 0, 18);
                BackgroundTransparency = 1;
            ...
            for _, status_name in ipairs(status_order) do
                ...
                status_buttons[status_name] = btn

                btn.MouseButton1Click:Connect(function()
                    cfg.set_status(status_name)
                end)
            end

            -- === Functions ===

            function cfg.update_status_buttons(status)
                for _, sname in ipairs(status_order) do
                    if sname == status then
                        library:tween(status_buttons[sname], {TextColor3 = status_colors[sname]})
                    else
                        library:tween(status_buttons[sname], {TextColor3 = rgb(86, 86, 87)})
                    end
                end
            end

            function cfg.add_player(player)
                if cfg.players[player.Name] then return end
                ...
                local team_name = player.Team ~= nil and player.Team.Name or "None"
                local team_color = rgb(255, 255, 255)
                if player.TeamColor then
                    team_color = BrickColor.new(tostring(player.TeamColor)).Color
                end

                row["team"] = library:create("TextLabel", {
                    FontFace = fonts.small;
                    TextColor3 = team_color;
                    BorderColor3 = rgb(0, 0, 0);
                    Text = team_name;
                    Parent = row["button"];
                    Name = "\0";
                ...
                row["status"] = library:create("TextLabel", {
                    FontFace = fonts.small;
                    TextColor3 = is_local and themes.preset.accent or rgb(130, 130, 130);
                    BorderColor3 = rgb(0, 0, 0);
                    Text = is_local and "LocalPlayer" or "Neutral";
                    Parent = row["button"];
                    Name = "\0";
                    Size = dim2(0.3, 0, 1, 0);
                    Position = dim2(0.7, 0, 0, 0);
                ...
                if is_local then
                    library:apply_theme(row["status"], "accent", "TextColor3")
                end

                row["liner"] = library:create("Frame", {
                    Parent = row["button"];
                    Name = "\0";
                    AnchorPoint = vec2(0, 1);
                    Position = dim2(0, 0, 1, 0);
                ...
                local pd = {
                    ...
                    player = player,
                    selected = false,
                    status = is_local and "LocalPlayer" or "Neutral",
                    items = row,
                }

                function pd.toggle(active)
                    if active then
                        library:tween(row["name"], {TextColor3 = themes.preset.accent})
                    else
                        library:tween(row["name"], {TextColor3 = rgb(245, 245, 245)})
                    end
                end
                ...
                function pd.set()
                    ...
                    if pd.selected then
                        cfg.current_player = player

                        for _, other in cfg.players do
                        ...
                        task.spawn(function()
                            ...
                            if ok and thumb then
                                items["avatar"].Image = thumb
                            end
                        end)

                        items["username"].Text = player.DisplayName .. " (@" .. player.Name .. ")"
                        items["userid"].Text = "ID: " .. tostring(player.UserId)
                        items["account_age"].Text = tostring(player.AccountAge) .. " days old"

                        cfg.update_status_buttons(pd.status)
                    else
                        cfg.current_player = nil
                        pd.toggle(false)

                        items["avatar"].Image = "rbxassetid://98200387761744"
                        items["username"].Text = "None"
                        items["userid"].Text = "None"
                        items["account_age"].Text = "None"

                        cfg.update_status_buttons(nil)
                    end

                    local ok, err = pcall(cfg.callback, cfg.current_player, pd.status, row["team"].Text)
                ...
                player:GetPropertyChangedSignal("Team"):Connect(function()
                    local tn = player.Team ~= nil and player.Team.Name or "None"
                    row["team"].Text = tn
                end)

                player:GetPropertyChangedSignal("TeamColor"):Connect(function()
                    if player.TeamColor then
                        row["team"].TextColor3 = BrickColor.new(tostring(player.TeamColor)).Color
                    end
                end)

                cfg.players[player.Name] = pd
                return pd
            end

            function cfg.remove_player(name)
                local pd = cfg.players[name]
                if pd then
                    pd.items["button"]:Destroy()
                    cfg.players[name] = nil

                    if cfg.current_player and cfg.current_player.Name == name then
                        cfg.current_player = nil
                        items["avatar"].Image = "rbxassetid://98200387761744"
                        items["username"].Text = "None"
                        items["userid"].Text = "None"
            ...
            function cfg.set_status(status_name)
                if not status_colors[status_name] then return end

                cfg.current_status = status_name

                if cfg.current_player and cfg.current_player ~= lp then
                    local pd = cfg.players[cfg.current_player.Name]
                    if pd and pd.status ~= "LocalPlayer" then
                        pd.status = status_name
                        library:tween(pd.items["status"], {TextColor3 = status_colors[status_name]})
                        pd.items["status"].Text = status_name

                        local ok, err = pcall(cfg.callback, cfg.current_player, status_name, pd.items["team"].Text)
                        if not ok then warn(err) end
                    end
                end

                cfg.update_status_buttons(status_name)
            end

            function cfg.refresh()
                for _, pd in cfg.players do
                    local tn = pd.player.Team ~= nil and pd.player.Team.Name or "None"
                    pd.items["team"].Text = tn
                    if pd.player.TeamColor then
                        pd.items["team"].TextColor3 = BrickColor.new(tostring(pd.player.TeamColor)).Color
                    end
                end
            end

            function cfg.clear_selection()
                if cfg.current_player then
                    local pd = cfg.players[cfg.current_player.Name]
                    if pd then
                        pd.selected = false
                        pd.toggle(false)
            ...
            -- Init: add all current players
            for _, player in players:GetPlayers() do
                cfg.add_player(player)
            end
            ...
            library:connection(players.PlayerRemoving, function(player)
                cfg.remove_player(player.Name)
            end)

            flags[cfg.flag] = "Neutral"
        ...
        function library:watermark(options)
            local cfg = {
                name = options.name or "Millenium",
                text = options.text or nil,
                fps = options.fps ~= false,      -- 默认显示 FPS
                time = options.time ~= false,    -- 默认显示时间
                visible = options.visible ~= false,
            }

            local wm_items = {}

            wm_items["outline"] = library:create("Frame", {
                Parent = library["items"];
                Name = "\0";
            ...
            wm_items["accent_line"] = library:create("Frame", {
                Parent = wm_items["outline"];
                Name = "\0";
            ...
            library:apply_theme(wm_items["accent_line"], "accent", "BackgroundColor3")

            library:create("UICorner", {
                Parent = wm_items["accent_line"];
                CornerRadius = dim(0, 2);
            })

            wm_items["inline"] = library:create("Frame", {
                Parent = wm_items["outline"];
                Name = "\0";
                Position = dim2(0, 1, 0, 3);
                Size = dim2(0, 0, 1, -4);
                BorderSizePixel = 0;
                BackgroundColor3 = rgb(22, 22, 24);
                AutomaticSize = Enum.AutomaticSize.X;
            ...
            -- background
            wm_items["background"] = library:create("Frame", {
                Parent = wm_items["inline"];
                Name = "\0";
                Position = dim2(0, 1, 0, 1);
                Size = dim2(0, 0, 1, -2);
                BorderSizePixel = 0;
            ...
            -- text label (RichText)
            wm_items["text"] = library:create("TextLabel", {
                Parent = wm_items["background"];
                Name = "\0";
                RichText = true;
                FontFace = fonts.font;
                TextColor3 = themes.preset.accent;
            ...
            library:apply_theme(wm_items["text"], "accent", "TextColor3")

            library:create("UIPadding", {
                Parent = wm_items["text"];
                PaddingLeft = dim(0, 8);
            ...
            library:draggify(wm_items["outline"])

            local fps_count = 0
            local last_tick = tick()

            function cfg.set_visible(bool)
                wm_items["outline"].Visible = bool
            end
            ...
            function cfg.update()
                local name_part = cfg.name
                local uid = tostring(game:GetService("Players").LocalPlayer.UserId)
                local text_color = "#91919b"

                if cfg.text then
                    name_part = name_part .. ' <font color="' .. text_color .. '">' .. cfg.text .. '</font>'
                end

                if cfg.time then
                    name_part = name_part .. ' <font color="' .. text_color .. '">/ ' .. os.date("%x %X") .. '</font>'
                end

                if cfg.fps then
                    name_part = name_part .. ' <font color="' .. text_color .. '">/ ' .. tostring(fps_count) .. ' fps</font>'
                end

                name_part = name_part .. ' <font color="' .. text_color .. '">/ UID ' .. uid .. '</font>'

                wm_items["text"].Text = name_part
            end

            library:connection(run.RenderStepped, function()
                if not wm_items["outline"].Visible then return end
                fps_count += 1
                local t = tick()
                if t - last_tick >= 1 then
                    last_tick = t
                    cfg.update()
                    fps_count = 0
                end
        ...
        library.keybinds_list = {
            visible = false,
            entries = {},
            items = {},
        }

        function library:create_keybinds_list()
            local kb = library.keybinds_list

            kb.items["outline"] = library:create("Frame", {
                Parent = library["items"];
                Name = "\0";
            ...
            -- accent 顶部装饰线
            kb.items["accent_line"] = library:create("Frame", {
                Parent = kb.items["outline"];
                Name = "\0";
                Position = dim2(0, 2, 0, 0);
                Size = dim2(1, -4, 0, 2);
            ...
            library:apply_theme(kb.items["accent_line"], "accent", "BackgroundColor3")

            library:create("UICorner", {
                Parent = kb.items["accent_line"];
                CornerRadius = dim(0, 2);
            })

            kb.items["inline"] = library:create("Frame", {
                Parent = kb.items["outline"];
                Name = "\0";
                Position = dim2(0, 1, 0, 3);
                Size = dim2(1, -2, 0, 0);
            ...
            kb.items["content"] = library:create("Frame", {
                Parent = kb.items["outline"];
                Name = "\0";
                Position = dim2(0, 1, 0, 3);
                Size = dim2(1, -2, 0, 0);
            ...
            kb.items["header"] = library:create("TextLabel", {
                Parent = kb.items["content"];
                Name = "\0";
                FontFace = fonts.font;
                TextColor3 = themes.preset.accent;
            ...
            library:apply_theme(kb.items["header"], "accent", "TextColor3")

            library:draggify(kb.items["outline"])
        end

        function library:show_keybinds_list()
            local kb = library.keybinds_list
            if not kb.items["outline"] then
                library:create_keybinds_list()
            end
            kb.visible = true
            kb.items["outline"].Visible = true
            library:tween(kb.items["outline"], {BackgroundTransparency = 0}, Enum.EasingStyle.Quad, 0.15)

            for _, entry in ipairs(kb.entries) do
                library:show_keybind_entry(entry)
            end
        end

        function library:hide_keybinds_list()
            local kb = library.keybinds_list
            if not kb.items["outline"] then return end
            kb.visible = false
            library:tween(kb.items["outline"], {BackgroundTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
            task.delay(0.16, function()
                kb.items["outline"].Visible = false
            end)
        end

        function library:show_keybind_entry(entry)
            local kb = library.keybinds_list
            if not kb.items["outline"] then return end
            if not entry.frame then return end
            entry.frame.Visible = true
            library:tween(entry.frame, {BackgroundTransparency = 0}, Enum.EasingStyle.Quad, 0.15)
            library:tween(entry.text, {TextTransparency = 0}, Enum.EasingStyle.Quad, 0.15)
            library:tween(entry.value, {TextTransparency = 0}, Enum.EasingStyle.Quad, 0.15)
        end

        function library:hide_keybind_entry(entry)
            local kb = library.keybinds_list
            if not entry.frame then return end
            library:tween(entry.frame, {BackgroundTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
            library:tween(entry.text, {TextTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
            library:tween(entry.value, {TextTransparency = 1}, Enum.EasingStyle.Quad, 0.15)
            task.delay(0.16, function()
                entry.frame.Visible = false
            end)
        end

        function library:register_keybind(name, cfg)
            local kb = library.keybinds_list
            if not kb.items["outline"] then
                library:create_keybinds_list()
            end
            ...
            entry.value = library:create("TextLabel", {
                Parent = entry.frame;
                Name = "\0";
                FontFace = fonts.font;
                TextColor3 = themes.preset.accent;
            ...
            library:apply_theme(entry.value, "accent", "TextColor3")

            entry.text = library:create("TextLabel", {
                Parent = entry.frame;
                Name = "\0";
                FontFace = fonts.font;
                TextColor3 = rgb(145, 145, 155);
                Text = name;
                Size = dim2(0, 0, 1, 0);
                BackgroundTransparency = 1;
                BorderSizePixel = 0;
            ...
            entry.update = function(active)
                entry.value.Text = active and "on" or "off"
            end

            entry.update(cfg.active)

            table.insert(kb.entries, entry)
            return entry
        end
        ...
        function library:init_config(window) 
            ...
            section:toggle({name = "Keybinds List", callback = function(bool)
                if bool then
                    library:show_keybinds_list()
                else
                    library:hide_keybinds_list()
                end
            end})
        end
    --
